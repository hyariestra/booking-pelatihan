					...
					<div class="boxform">
						...
						<div id="step-3">
							...
							<div id="form-step-0" role="form" data-toggle="validator">
								...
								<div class="col-md-12">
									...
									foreach ($produk->result_array() as $key => $value) {
										...
										?>
										<div class="radio">
											<label class="spanlabel"><input onclick="getValueRadio(this);addrowtableNon(this); show<?php echo $value['id_produk_want']?>();" type="radio" value="<?php echo $value['id_produk_want'] ?>#<?php echo $value['harga_produk'] ?>" name="produk"><?php echo $value['nama_produk'] ?></label>
										</div>
							...
							<div class="row" style="background:#eee;color:#555;margin:0px;">
								...
								<div class="col-md-3">
									<button disabled="" id="disbutt" type="button" onclick="showmodalharga();hitungjumlah(this);" style="color:#fff;" class="btn btn-sm pull pull-right">
...
<script type="text/javascript">
	...
	*/
	...
	function addrowtableNon(obj) 
	{
		...
		$("#tableins").find('tr[id="removeable"]').remove();
		var kategori = $("input[name='instansi']:checked").val();
		kategori = (kategori==undefined)?'0#0': kategori
		var selected = $(obj).is(":checked");
		var idx = $(obj).val();
		var idsplit = idx.split("#");
		var target = "<?php echo site_url('personil/getharganon'); ?>"
		...
		$.post(target,data,function(e){
			...
			for(i = 0; i < json.non.length; i++)
			{
				var id_personil = json.non[i].id_personil;
				var keterangan = json.non[i].keterangan;
				var satuan = json.non[i].satuan;
				var harga = json.non[i].harga;
				var people = $("input[name='jmlpeserta']").val();
				var lamane = $("#info_lamapelatihan").text(); 
				var volum = eval(people)*diffDays;
				var sumharga = volum*harga;

			var table = document.getElementById('tableins');
			var rowNon = table.insertRow(5);
			rowNon.id = 'removeable';
			var Seq = '<input type="hidden" name="idp" value="'+id_personil+'">  ';
			var colbody0 = rowNon.insertCell(0);  
			var colbody1 = rowNon.insertCell(1);  
			var colbody2 = rowNon.insertCell(2);  
			var colbody3 = rowNon.insertCell(3);  
			var colbody4 = rowNon.insertCell(4);  
			var colbody5 = rowNon.insertCell(5);  
			var colbody6 = rowNon.insertCell(6);  
			var colbody7 = rowNon.insertCell(7);  

			if (id_personil==4) {
				var volharga  = people*harga;
				var lamane = '1 Hari';
				var volum = people;
				var sumharga = people*harga;
			}else if(id_personil == 5) {
				var volharga = volum*harga;
	...
	function hitungjumlah(obj)
	{
		...
		$.each(table,function(i,v){
			...
			var colbody1 = rowbody.insertCell(0);  
			
			colbody1.style.fontWeight = "bold";
			

			colbody1.colSpan = 7;
			colbody1.innerHTML = 'Total'; 
			colbody0.innerHTML = total; 
